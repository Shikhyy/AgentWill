...
function BaseIcon({ className = "" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 120 120" fill="none">
      <circle cx="60" cy="60" r="55" fill="#0052FF" opacity="0.1"/>
      <circle cx="60" cy="60" r="35" fill="#0052FF" opacity="0.2"/>
      <text x="60" y="67" textAnchor="middle" fill="#0052FF" fontSize="32" fontWeight="bold" fontFamily="sans-serif">b</text>
    </svg>
  )
}
...
function EthIcon({ className = "" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 256 417" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path fill="#d1d1d1" d="M127.962 0L0 212.32l127.962 75.639V154.158z"/>
      <path fill="#8c8c8c" d="M127.961 312.187l-1.575 1.92v98.199l1.575 4.6L256 236.587z"/>
      <path fill="#d1d1d1" d="M127.962 416.905V312.186L0 236.587z"/>
      <path fill="#627eea" d="M127.962 212.32v103.866l127.958-75.639z" fillOpacity=".602"/>
    </svg>
  )
}
    ...
    <nav className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${scrolled ? 'glass py-3' : 'py-5'}`}>
      ...
      {mobileOpen && (
        <div className="md:hidden glass mt-2 mx-4 rounded-2xl p-6 flex flex-col gap-4">
          <a href="#features" className="text-slate-300 hover:text-white" onClick={() => setMobileOpen(false)}>Features</a>
          <a href="#how-it-works" className="text-slate-300 hover:text-white" onClick={() => setMobileOpen(false)}>How it Works</a>
          <a href="#demo" className="text-slate-300 hover:text-white" onClick={() => setMobileOpen(false)}>Demo</a>
          <a href="https://github.com/Shikhyy/AgentWill" target="_blank" rel="noopener noreferrer" className="text-slate-300 hover:text-white">GitHub</a>
          <a href="#demo" className="btn-primary text-center" onClick={() => setMobileOpen(false)}>Launch App</a>
        </div>
      )}
    </nav>
  )
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden hero-gradient">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 text-center pt-32 pb-20">
        ...
        <div className="flex flex-wrap items-center justify-center gap-8 mt-16 animate-slide-up" style={{ animationDelay: '450ms' }}>
          {[
            { icon: <BaseIcon className="w-5 h-5" />, label: 'Built on Base' },
            { icon: <CodeIcon className="w-5 h-5" />, label: 'Solidity 0.8.24' },
            { icon: <ShieldIcon className="w-5 h-5" />, label: 'ERC-8004 Compatible' },
            { icon: <ActivityIcon className="w-5 h-5" />, label: '8 Passing Tests' },
          ].map((item, i) => (
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 animate-bounce">
        <div className="w-6 h-10 rounded-full border-2 border-slate-600 flex items-start justify-center p-2">
          <div className="w-1.5 h-3 rounded-full bg-slate-500 animate-pulse" />
        </div>
      </div>
    </section>
  )
          ...
          {features.map((feature, i) => {
            ...
            return (
              <AnimatedSection key={i} delay={i * 100} className="group">
                <div className={`glass rounded-2xl p-6 h-full card-hover border ${c.border} hover:${c.glow}`}>
                  <div className={`w-14 h-14 rounded-xl ${c.bg} border ${c.border} flex items-center justify-center mb-5 ${c.text}`}>
                    {feature.icon}
                  </div>
  ...
  const steps = [
    {
      ...
      description: "Set spending limits, interaction permissions, and privacy controls through a clean interface. No code required.",
      detail: "Choose tokens, daily limits, per-transaction caps, and which contracts your agent can interact with.",
    },
    {
      number: "02",
      title: "Deploy to Base",
      description: "Your rules are compiled and deployed as an immutable smart contract on Base mainnet.",
      detail: "Rules are on-chain forever. No backend, no servers. Your rules survive any platform.",
    },
    {
      number: "03",
      title: "Agent Operates Autonomously",
      description: "Your AI agent operates freely within the rules you set. Every action is verified against the contract.",
      detail: "Before each action, the agent queries the contract. The contract either allows or blocks — permanently logged.",
    },
    {
      number: "04",
      title: "You Stay in Control",
      description: "Monitor all activity in real-time. Update rules anytime. Revoke access instantly if needed.",
      detail: "Toggle rules on/off, adjust limits, add/remove targets — all from your dashboard.",
    },
  ]
              ...
              <div className="flex flex-col md:flex-row items-center gap-8 md:gap-12">
                <div className="flex-1 text-center md:text-left">
                  ...
                  <p className="text-orange-400 leading-relaxed mb-6">
                    ...
                    <br /><br />
                    AgentWill is that layer. A signed, on-chain rule set that defines exactly what the agent can do — 
                    and what it can't.
                  </p>
...
function StatsSection() {
  const stats = [
    { value: "Base", label: "Chain", suffix: "L2" },
    { value: "8", label: "Tests", suffix: "Passing" },
    { value: "3", label: "Contracts", suffix: "Deployed" },
    { value: "ERC", label: "Standard", suffix: "8004" },
  ]
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {stats.map((stat, i) => (
            <AnimatedSection key={i} delay={i * 100} className="text-center">
              <div className="text-3xl md:text-4xl font-black gradient-text mb-1">{stat.value}{stat.suffix}</div>
...
function DemoSection() {
  ...
  const [spendingRules, setSpendingRules] = useState([{ token: 'USDC', dailyLimit: '1000', perTxLimit: '100' }])
  const [interactionRules, setInteractionRules] = useState([{ target: '', allowed: true }])
  const [revealMetadata, setRevealMetadata] = useState(false)
  const [submitted, setSubmitted] = useState(false)
  const [agentId, setAgentId] = useState('')

  const addSpendingRule = () => setSpendingRules([...spendingRules, { token: 'ETH', dailyLimit: '0.5', perTxLimit: '0.1' }])
  const removeSpendingRule = (i: number) => setSpendingRules(spendingRules.filter((_, idx) => idx !== i))
  const updateSpendingRule = (i: number, field: string, value: string) => {
    const updated = [...spendingRules]
    updated[i] = { ...updated[i], [field]: value }
    setSpendingRules(updated)
  }

  const addInteractionRule = () => setInteractionRules([...interactionRules, { target: '', allowed: true }])
  const removeInteractionRule = (i: number) => setInteractionRules(interactionRules.filter((_, idx) => idx !== i))
  const updateInteractionRule = (i: number, field: string, value: any) => {
          ...
          <div className="glass rounded-3xl overflow-hidden border border-rose-500/20">
            <div className="flex border-b border-white/5">
              {['create', 'manage'].map(tab => (
                ...
                >
                  {tab === 'create' ? 'Create Rules' : 'Manage Rules'}
                </button>
              ))}
            </div>
            ...
            <div className="p-8">
              {activeTab === 'create' && !submitted && (
                <div className="space-y-8">
                  <div>
                    <label className="block text-sm font-medium mb-3">Agent ID / Address</label>
                    <input
                      type="text"
                      placeholder="0x... (your agent's on-chain address)"
                      value={agentId}
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {spendingRules.map((rule, i) => (
                        <div key={i} className="flex items-center gap-3 bg-[#0a0f1e] rounded-xl p-4">
                          <select
                            value={rule.token}
                          ...
                          <div className="flex-1">
                            <label className="text-xs text-slate-500">Per-Tx Limit</label>
                            <input
                              type="text"
                    ...
                    <div className="space-y-3">
                      {interactionRules.map((rule, i) => (
                        <div key={i} className="flex items-center gap-3 bg-[#0a0f1e] rounded-xl p-4">
                          <input
                            type="text"
                            placeholder="Contract address (e.g. Uniswap Router)"
                            value={rule.target}
                            onChange={e => updateInteractionRule(i, 'target', e.target.value)}
                            className="flex-1 !py-2.5 !px-4 !text-sm"
                          />
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <span className="text-xs text-slate-500">Allow</span>
                            <button
                              onClick={() => updateInteractionRule(i, 'allowed', !rule.allowed)}
                              className={`w-12 h-6 rounded-full transition-colors relative ${rule.allowed ? 'bg-emerald-500' : 'bg-slate-700'}`}
                            >
                              <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-all ${rule.allowed ? 'left-6' : 'left-0.5'}`} />
                            </button>
                          </div>
                  ...
                  <div className="flex items-center justify-between p-5 bg-[#0a0f1e] rounded-xl border border-white/5">
                    <div>
                      <h4 className="font-semibold">Reveal Metadata On-Chain</h4>
                      <p className="text-xs text-slate-500 mt-1">Allow spending patterns to be publicly visible</p>
                    </div>
                    <button
    ...
    <section className="relative py-20 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-6">
        <AnimatedSection className="text-center mb-12">
          <h3 className="text-2xl font-bold mb-2">Built With</h3>
          <p className="text-slate-500 text-sm">Modern, battle-tested technology</p>
        </AnimatedSection>

        <div className="flex flex-wrap justify-center gap-4">
          {tech.map((t, i) => (
            <AnimatedSection key={i} delay={i * 80}>
              <div className="glass rounded-xl px-5 py-3 flex items-center gap-3 card-hover">
...
function HackathonSection() {
  ...
  return (
    <section className="relative py-24 overflow-hidden">
      <FloatingOrb size={400} color="#10b981" position="center" blur={150} opacity={0.06} />
      ...
      <div className="relative max-w-4xl mx-auto px-6 text-center">
        <AnimatedSection>
          ...
          <p className="text-slate-500 text-sm">
            AgentWill addresses the core problem of AI agent accountability — giving humans enforceable control over autonomous agents through on-chain rule sets.
          </p>
        </AnimatedSection>
      </div>
    </section>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-slate-500">On-chain rules for AI agents</div>
            </div>
          </div>
